...
> Source: `TakeHome.docx` (Gauntlet take-home project brief). This document preserves the full brief, including stakeholder interview notes, since requirements are embedded throughout.

## Overview

This application will have the following main parts:

1. **Application-to-label matching** — shows the percentage matched to what is on the label, what doesn't match, and why.
2. **Label and application generator** — generates a mock application with label.

Read the interviews and criteria below and ensure the application fulfills everything.

### Reference links

- [TTB Certificate of Label Approval (COLA)](https://www.ttb.gov/alfd/certificate-of-label-aproval-cola)
- [TTB Permits Online — preview applications for new permits, registrations, and notices](https://www.ttb.gov/online-services/ponl/ponl-help/preview-applications-for-new-permits-registrations-and-notices#Importer/Wholesaler)
- [COLA public registry search](https://ttbonline.gov/colasonline/publicSearchColasBasicProcess.do?action=search)

### Reference files (in this repo)

- [LabelExample1.pdf](LabelExample1.pdf), [LabelExample2.pdf](LabelExample2.pdf), [LabelExample3.pdf](LabelExample3.pdf) — examples of applications and labels.

---

## Project Background & Stakeholder Context

Notes from discovery sessions with the Compliance Division, along with technical requirements for the prototype. Stakeholder feedback is included for context on how this tool will be used.

### Interview: Sarah Chen, Deputy Director of Label Compliance
...
### Interview: Marcus Williams, IT Systems Administrator
...
### Interview: Dave Morrison, Senior Compliance Agent (28 years)
...
### Interview: Jenny Park, Junior Compliance Agent (8 months)
...
You are free to use any programming languages, frameworks, or libraries you prefer. The evaluators want to see what kind of engineering, design, and integration decisions you make.
...
TTB requires specific information on alcohol beverage labels. The exact requirements vary by beverage type (beer, wine, distilled spirits) but common elements include:
...
Review TTB's guidelines at [ttb.gov](https://www.ttb.gov) for additional context on label requirements.

### Sample Label

The app should handle labels containing information like this example:
...
We are encouraged to create or source additional test labels — AI image generation tools work well for this.
...
2. **Deployed Application URL**
   - Working prototype the evaluators can access and test
...
> "We understand this is time-constrained. A working core application with clean code is preferred over ambitious but incomplete features. Document any trade-offs or limitations. Questions? Reach out for clarification—though we also value how you fill in gaps independently. Good luck!"

---

## Derived Requirements (extracted from interviews)

These are implicit requirements buried in the stakeholder notes, made explicit:

| # | Requirement | Source |
|---|---|---|
| R1 | Application-to-label match with **match percentage**, list of mismatches, and **reasons why** | Brief |
| R2 | Mock **application + label generator** | Brief |
| R3 | Verification results in **~5 seconds or less** per label | Sarah (failed vendor pilot) |
| R4 | UI must be **extremely simple and obvious** — usable by non-technical, older users; no hunting for buttons | Sarah |
| R5 | **Batch upload** support (200–300 applications at once during peak season) | Sarah |
| R6 | **Fuzzy/judgment matching** — e.g., "STONE'S THROW" vs "Stone's Throw" should not be a hard mismatch; explain nuance | Dave |
| R7 | **Government warning statement** must be verified **word-for-word**, with "GOVERNMENT WARNING:" in **all caps and bold** | Jenny |
| R8 | Standalone proof-of-concept — **no COLA integration** | Marcus |
| R9 | No sensitive data storage required; reasonable (not extreme) security for a prototype | Marcus |
| R10 | Be mindful of restrictive networks/firewalls when relying on cloud APIs (noted constraint, prototype is deployed publicly) | Marcus |
| R11 | *Stretch:* handle imperfect label images (angles, glare, bad lighting) | Jenny |

---

## Frontend Design Reference

The frontend build references the local **house-style** design system at `C:\Users\jaynyasg\OneDrive\Documents\GitLab\house-style`:

- **Aesthetic:** calm, technical, light-mode editorial — a *tool*, not a consumer app. Off-white canvas (`#f6f7f9`), white cards, near-black ink, color reserved for meaning.
- **Usage:** copy `tokens/tailwind.preset.ts` as a Tailwind preset, copy `components/*.tsx` patterns (Button, IconButton, Chip, Stepper, Card, Badge, Modal, icons), and apply the base layer from `preview/src/index.css` (Inter Tight, tracking `-0.011em`, font features, utilities).
- It is a source to **copy from, not a build dependency** — this project gets its own copy that may diverge.
- See `house-style/README.md` for the full design language and `references/UI.png` + `preview/` for visual reference.
